...
namespace cuda {
	...
	private:
		...
		public:
			...
			data_context(const config_t config, const cuda::model& model, const std::vector<data::sample<float>>& samples);
			void synchronize(const config_t config, const cuda::model& model, const std::vector<data::sample<float>>& samples);
		};

		bool train_sample(const int i, const data::sample<float>& sample, data_context& context, cuda::model& model);
		bool test_sample(const int i, const data::sample<float>& sample, data_context& context, cuda::model& model);
