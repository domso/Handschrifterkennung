...
namespace cuda {
	...
	neuronal_network::neuronal_network() : m_currentContext(nullptr){
	...
	void neuronal_network::train(cuda::model& model, std::vector<data::sample<float>>& trainingsData) {
		std::vector<float>& refInput = trainingsData[0].internal_data();

		model.init((refInput.size() + 1) * m_currentConfig.numHidden
				+ (m_currentConfig.numHidden + 1) * m_currentConfig.numOutput);
	...
	void neuronal_network::set_classify_context(cuda::model& model, data::sample<float>& s) {
	...
	: hiddenLayer(config.numHidden),
	outputLayer(config.numOutput),
	labels(samples.size() * outputLayer.size()),
	devInput(samples[0].internal_data(), samples.size()),
	devHidden(hiddenLayer),
	devOutput(outputLayer),
	devWeights(model.get_weights()),
	devLabels(labels)
	{
	...
	void neuronal_network::train_data_context::synchronize(const config_t config, const cuda::model& model, const std::vector<data::sample<float>>& samples) {
		...
		//for (int i = 0; i < samples.size(); i++) {
			//devInput.synch_to_device(samples[i].internal_data(), i);
		//}
